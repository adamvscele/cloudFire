...
- (instancetype)initWithFrame:(CGRect)frame{
    self = [self initWithFrame:frame withCenterY:frame.size.height/2];
    return self;
}

- (instancetype)initWithFrame:(CGRect)frame withCenterY:(float)centerY{
...
- (void)drawRect:(CGRect)rect {
    ...
    //勾股定理算出圆心距离的对角线距离
    CGFloat standardRadius = sqrt(pow(_center.x, 2) + pow(self.bounds.size.height - _center.y, 2));
...
//        keyFrameAnimation.autoreverses = YES;//翻转动画
        keyFrameAnimation.speed = self.speed/2 - i * 0.21;
...
//        NSLog(@"%@",keyFrameAnimation.values);
        ...
        i++;
        changeRadius = changeRadius + 5 * (6 + i * 3);
    }
